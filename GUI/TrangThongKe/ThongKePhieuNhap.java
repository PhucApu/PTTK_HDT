...
public class ThongKePhieuNhap extends JFrame {
	...
	private JLabel GanNCCNhapMax_label;
	...
	public ThongKePhieuNhap(TrangThongKe_view trangThongKe_view) {
		...
		JLabel ThongKePhieuNhap_label = new JLabel("THỐNG KÊ PHIẾU NHẬP", JLabel.CENTER);
		ThongKePhieuNhap_label.setForeground(new Color(97, 113, 67));
		ThongKePhieuNhap_label.setFont(new Font("Tahoma", Font.BOLD, 35));
		ThongKePhieuNhap_label.setBounds(10, 23, 785, 40);
		contentPane.add(ThongKePhieuNhap_label);

		BangThongKeSP_label = new JLabel("Bảng thống kê phần trăm các sản phẩm được nhập", JLabel.CENTER);
		...
		JLabel PNTongTienLonNhat_label = new JLabel("Phiếu nhập có tổng tiền lớn nhất :");
		PNTongTienLonNhat_label.setFont(new Font("Tahoma", Font.BOLD, 12));
		PNTongTienLonNhat_label.setBounds(10, 28, 239, 30);
		panel.add(PNTongTienLonNhat_label);
		...
		JLabel NVNhapMax_label = new JLabel("Nhân viên nhập nhiều nhất :");
		NVNhapMax_label.setFont(new Font("Tahoma", Font.BOLD, 12));
		NVNhapMax_label.setBounds(10, 68, 199, 30);
		panel.add(NVNhapMax_label);

		GanNVNhapMax_label = new JLabel("....");
		GanNVNhapMax_label.setFont(new Font("Tahoma", Font.BOLD, 12));
		GanNVNhapMax_label.setBounds(195, 71, 165, 30);
		panel.add(GanNVNhapMax_label);

		JLabel NCCNhapMax_label = new JLabel("Nhà cung cấp nhập nhiều nhất :");
		NCCNhapMax_label.setFont(new Font("Tahoma", Font.BOLD, 12));
		NCCNhapMax_label.setBounds(10, 111, 211, 30);
		panel.add(NCCNhapMax_label);

		GanNCCNhapMax_label = new JLabel("....");
		GanNCCNhapMax_label.setFont(new Font("Tahoma", Font.BOLD, 12));
		GanNCCNhapMax_label.setBounds(215, 111, 145, 30);
		panel.add(GanNCCNhapMax_label);

		JLabel BangThongKePN_label = new JLabel("Bảng thống kê phiếu nhập", JLabel.CENTER);
		BangThongKePN_label.setFont(new Font("Tahoma", Font.BOLD, 15));
		BangThongKePN_label.setBounds(419, 83, 350, 30);
		contentPane.add(BangThongKePN_label);
		...
		// controller
		controller_btnThongKeTGPhieuNhap controller_btnThongKeTGPhieuNhap = new controller_btnThongKeTGPhieuNhap(
				this);
		TimKiemICon_label.addMouseListener(controller_btnThongKeTGPhieuNhap);
		controller_ThoatTKPhieuNhap controller_ThoatTKPhieuNhap = new controller_ThoatTKPhieuNhap(this);
		btn_ThoatPN.addMouseListener(controller_ThoatTKPhieuNhap);
		controller_btnXuatExcel_TKPhieuNhap controller_btnXuatExcel_TKPhieuNhap = new controller_btnXuatExcel_TKPhieuNhap(this);
		btn_XuatExcel.addMouseListener(controller_btnXuatExcel_TKPhieuNhap);
		controller_btnDocExcel_TKPhieuNhap controller_btnDocExcel_TKPhieuNhap = new controller_btnDocExcel_TKPhieuNhap(this);
		btn_DocExcel.addMouseListener(controller_btnDocExcel_TKPhieuNhap);
		setVisible(true);
	}
	...
	public void showThongKe() {
		...
		ArrayList<Double> phantram = trangThongKe_view.getThongKe().ThongKe_nhapProduct("1/1/2000",
				Sys.getDateNow());
		int index = 0;
		...
		this.GanPNTongTienLonNhat_label.setText(
				trangThongKe_view.getThongKe().ThongKe_PhieuNhapTongTienMax("1/1/2000", Sys.getDateNow())
						.toString());
		this.GanNCCNhapMax_label
				.setText(trangThongKe_view.getThongKe().ThongKe_nhapNCCmax("1/1/2000", Sys.getDateNow())
						.toString());
		this.GanNVNhapMax_label.setText(
				trangThongKe_view.getThongKe().ThongKe_NhanVienNhapMax("1/1/2000", Sys.getDateNow())
						.toString());
	}
	...
	public void btn_ThongKeTG() {
		...
		if (Sys.compareDay(dayBefore, dayAfter)) {
			ListProductsBUS listProducts = trangThongKe_view.getThongKe().getListProducts();
			ArrayList<Double> phantram = trangThongKe_view.getThongKe().ThongKe_nhapProduct(dayBefore,
					dayAfter);
			int index = 0;
			...
			this.GanPNTongTienLonNhat_label.setText(trangThongKe_view.getThongKe()
					.ThongKe_PhieuNhapTongTienMax(dayBefore, dayAfter).toString());
			this.GanNCCNhapMax_label.setText(
					trangThongKe_view.getThongKe().ThongKe_nhapNCCmax(dayBefore, dayAfter).toString());
			this.GanNVNhapMax_label.setText(trangThongKe_view.getThongKe()
					.ThongKe_NhanVienNhapMax(dayBefore, dayAfter).toString());
	...
	public void btn_xuatFile_Excel() throws IOException {
		String dayBefore = this.DayBefore_textField.getText();
		String dayAfter = this.DayAfter_textField.getText();
		String pathname = "D:\\A\\JAVA\\DO_AN_JAVA_SWING\\Excel_PhieuNhap\\" + "TK_PN_("
				+ Sys.takeDayFormat_(dayBefore) + "-" + Sys.takeDayFormat_(dayAfter) + ").xlsx";
		if (Sys.createExcelFile(pathname)) {
			...
			ArrayList<String> maPhieu = trangThongKe_view.getThongKe()
					.ThongKe_PhieuNhapTongTienMax(dayBefore, dayAfter);
			// ma nhan vien
			ArrayList<String> maNV = trangThongKe_view.getThongKe().ThongKe_NhanVienNhapMax(dayBefore,
					dayAfter);
			// ma nha cung cap
			ArrayList<String> maNCC = trangThongKe_view.getThongKe().ThongKe_nhapNCCmax(dayBefore, dayAfter);

			if (Sys.writeExcel_TKPhieuNhap(pathname, ngaythongke, maSP, nameSP, phantram, maPhieu, maNV,
					maNCC)) {
	...
	public void btn_readExcelFile_and_check() throws IOException {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			String pathname = "D:\\A\\JAVA\\DO_AN_JAVA_SWING\\Excel_PhieuNhap\\" + chooser.getSelectedFile().getName();
			...
			ArrayList<String> phieunhaptongtienmax = Sys.readExcel_PhieuNhapCoTongTienLonNhat(pathname);
			ArrayList<String> maNV = Sys.readExcel_NhanVienNhapNhieuPhieuNhat(pathname);
			ArrayList<String> maNCC = Sys.readExcel_NhaCungCapNhapNhieuNhat(pathname);
			...
			// phan tram
			ArrayList<Double> phantram_data = trangThongKe_view.getThongKe().ThongKe_nhapProduct(dayBefore,
					dayAfter);
			// ma phieu
			ArrayList<String> maPhieu = trangThongKe_view.getThongKe()
					.ThongKe_PhieuNhapTongTienMax(dayBefore, dayAfter);
			// ma nhan vien
			ArrayList<String> maNV_data = trangThongKe_view.getThongKe().ThongKe_NhanVienNhapMax(dayBefore,
					dayAfter);
			// ma nha cung cap
			ArrayList<String> maNCC_data = trangThongKe_view.getThongKe().ThongKe_nhapNCCmax(dayBefore, dayAfter);

			// so sanh

			int flag = 0;		// kiem tra

			if(maSP.equals(maSP_data) == true && nameSP.equals(nameSP_data) ==  true && phantram.equals(phantram_data) ==  true && phieunhaptongtienmax.equals(maPhieu) == true && maNV.equals(maNV_data) == true && maNCC.equals(maNCC_data) == true){
				flag = 1;
			}
